...
namespace bw
{
	namespace core
	{
		...
		World::World(float size, float gridSize, WorldDataGeneratorFactory generatorFactory)
			: mExtents(-size / 2, -size / 2, size, size)
			, mPlayerStartPosition{ 0.0f, 0.0f }
			, mPlayerStartAngle(0.0f)
			, mAlwaysUpdateVertices(false)
			, mFrameNumber(0)
			, mDataGenerator(nullptr)
			, mPrimitiveLookupGrid(nullptr)
			, mTriggerLookupGrid(nullptr)
			, mPrevPlayerPosition{ 999999.0f, 999999.0f }
			, mPrefabAreaTilingType(PrefabAreaTilingType::None)
			, mPrefabAreaTileTypes(0)

...
bw::core::PrimitiveFieldLayout representativeLayout() {
  auto result = bw::core::buildBoundedPrimitiveFieldLayout(
      ...
      {{70.0f, 20.0f}, {-60.0f, -25.0f}, {0.0f, 0.0f}});
  require(result.succeeded(), "representative bounded layout failed");
  return std::move(*result.layout);
}

std::vector<editor::PrimitiveFieldRectanglePreview> previews(
    bw::core::PrimitiveFieldLayout const& layout,
    float overlap = 10.0f,
    int seed = 19) {
  auto result = editor::buildPrimitiveFieldRectanglePreview(
      layout, overlap, seed);
  require(result.succeeded(), "representative Rectangle preview failed");
  return std::move(*result.rectangles);
}
...
void placementAppendsDefaultsAndIsOneUndoableAction() {
  ...
  editor::_setPrimitiveParameters(
      &expected, 0, 0, {}, {}, rectangles[0].size, rectangles[0].angle);
  ...
  for (size_t i = 0; i < rectangles.size(); ++i) {
    ...
    auto primitive = dynamic_cast<bw::core::RectanglePolygon*>(
        world->getPrimitive(index));
    require(primitive && primitive->getPosition() == layout.sites[i] &&
                primitive->getSize() ==
                    wp::Vector2{rectangles[i].size, rectangles[i].size} &&
                primitive->getXyRatio() ==
                    editor::PrimitiveFieldRectangleXyRatio,
            "Rectangle insertion order, position, or size did not match preview");
    require(primitive->getOperation() ==
            ...
            "a placed Rectangle did not use normal editor defaults");
    require(sameProperties(primitive->getProperties(), expected.getProperties()),
            "placed Rectangle materials did not match normal editor creation");

    auto const angle = rectangles[i].angle;
    ...
    require(transformed.size() == 1 && transformed[0].size() == 1 &&
                transformed[0][0].size() == rectangles[i].contour.size(),
            "placed Rectangle did not produce one complete contour");
    for (size_t vertex = 0; vertex < rectangles[i].contour.size(); ++vertex) {
      auto delta = transformed[0][0][vertex].p - rectangles[i].contour[vertex];
  ...
  for (size_t i = 0; i < placedAngles.size(); ++i) {
    require(document.getWorld()
            ...
            "Redo did not restore the identical Rectangle field");
  }
}
...
void insertionFailureIsAtomicAndDoesNotTouchHistory() {
  ...
  auto layout = representativeLayout();
  auto rectangles = previews(layout, 0.0f, 4);
...
void invalidBatchFailsBeforeInsertionAndCapacitySettingIsHonoured() {
  ...
  auto layout = representativeLayout();
  auto rectangles = previews(layout);
  rectangles.back().angle = std::numeric_limits<float>::quiet_NaN();
  ...
  auto validRectangles = previews(layout);
  ...
  result = editor::placePrimitiveField(
      &document, layout, validRectangles, noCreateGeneration);

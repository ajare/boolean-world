...
// Route asserts through the editor's logger. The default assert() aborts with
// its message going nowhere in a windowed process, which makes an ImGui
// misuse indistinguishable from any other crash. Defined in Main.cpp.
void EditorImGuiAssertFailed(char const* expr, char const* file, int line);
#define IM_ASSERT(_EXPR)                                     \
  do {                                                       \
    if (!(_EXPR)) {                                          \
      EditorImGuiAssertFailed(#_EXPR, __FILE__, __LINE__);   \
    }                                                        \
  } while (0)

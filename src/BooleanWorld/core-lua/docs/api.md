...
A `RunScript` step stores these authored values:
...
| `set_floor_elevation(angle, lower, upper)` | Sets the floor Elevation span. Zero degrees points along local +Y and angles increase counter-clockwise; elevation interpolates from `lower` to `upper` across the fitted bounds. All values must be finite. |
...
| `get_ceiling_material()` | Returns the ceiling Sub-material id. |
...
| `get_floor_elevation()` | Floor `angle, lower, upper`. |
| `get_ceiling_elevation()` | Ceiling `angle, lower, upper`. |

...
**Chip**:
A tapered chamfer cut into a convex Arris: a wedge of material removed at its deepest near the Chip's centre, tapering to nothing at both ends. Derived after the boolean fold from the Arrangement alone — never authored, and never written to a World file, which holds no record of any Chip. Purely visual: collision, floor height, face containment and surface picking all continue to see the unchipped world. The wall's Sub-material governs the minimum Arris length, probability, spacing, and depth/reach ranges of the Chips its Arris may carry, and also supplies each new facet's material. Reach is the Chip's total extent along the Arris; width is half its reach and may never exceed 2 world units. Only convex Arrises are eligible — a FloorStep's top or a CeilingStep's bottom — because a concave one would have to carve into solid material rather than remove a wedge from it.

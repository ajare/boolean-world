...
A per-edge flag on a MeshPrimitive's Ring, editable in the editor's Edge sub-mode only for an edge used by exactly one polygon in that Primitive's own mesh topology (Willpower's `External` edge connectivity — an edge used by two polygons, or by zero/more than two, is fixed off and not editable). Defaults on for an eligible edge. Where that edge, or any sub-segment it is split into during the fold, produces an ArrangementWall (Border or Step), the flag is the sole determinant of that wall segment's collision — it replaces the geometry-computed Border-always-blocks / step-threshold rule for that segment, but never creates a wall where the fold produces none.

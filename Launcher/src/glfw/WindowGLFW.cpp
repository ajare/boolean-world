...
WindowGLFW::WindowGLFW(string const& title, ProgramOptions const& options)
    : Window(title, options), mWindow(nullptr), mContentScale(1.0f) {
}
...
void WindowGLFW::create() {
  ...
  glfwSetCursorEnterCallback(mWindow, WindowGLFW::mouseEnterCallback);
  checkGlfwError();
  glfwSetWindowFocusCallback(mWindow, WindowGLFW::focusCallback);
  checkGlfwError();
  glfwSetKeyCallback(mWindow, WindowGLFW::keyCallback);
  checkGlfwError();
  glfwSetMouseButtonCallback(mWindow, WindowGLFW::mouseButtonCallback);
  checkGlfwError();
  glfwSetCursorPosCallback(mWindow, WindowGLFW::mousePosCallback);
  checkGlfwError();
  glfwSetScrollCallback(mWindow, WindowGLFW::scrollCallback);
  checkGlfwError();
  glfwSetCharCallback(mWindow, WindowGLFW::charCallback);
  checkGlfwError();
